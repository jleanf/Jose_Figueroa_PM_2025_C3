...
void main(void)
{
    ...
    reemplazofflush(); //fflush(stdin); <--- Modificado (no funciona)
    ...
    if (isalpha (p1))
        if (islower (p1))
            /* La función islower regresa 1 si p1 es una letra minuscula y 0 en caso contrario.
            La función isupper, por otra parte, regresa 1 si p1 es una letra mayuscula y 0 en caso contrario. */
    ...
    /* La función tolower convierte de mayuscula a minuscula. Si la letra es minuscula no la modifica.
    La función toupper, por otra parte, convierte de minuscula a mayuscula. Si la letra es mayuscula no la
    modifica. */
        else
            printf("%c es una letra minuscula \n", p1);
    else
